...
# Within Jupyter Lab, a folder entitled "data" was created.
# A folder for each exchange wasthen created within the "data" folder.
# Each exchange folder then included csv files for all of the trading pairs
# supported on that respective exchange and for all intervals (300 and 3600).
def insert_csv_to_db(credentials):
    '''
    Connects to a PostgreSQL database and imports csv
    files into a specified schema and table name.
    '''
        ...
        if directory != '.DS_Store':
            for filename in os.listdir('data/' + directory):
                ...
                with open('data/' + directory + '/' + filename, 'r') as f:
                    # Skip the header row.
                    next(f)
                    cur.copy_from(f, '{schema}.{table_name}'.format(
                        schema=schema, table_name=table_name), sep=','
                        )
                    conn.commit()
...
def drop_column(credentials):
    '''
    Connects to a PostgreSQL database and drops a table column,
    in this case ID, from all tables within each schema.
    '''
    ...
    for schema in schemas:
        for table_name in table_list:
            cur.execute(
                '''ALTER TABLE {schema}.{table_name} DROP COLUMN ID;'''
                .format(schema=schema, table_name=table_name))
