...
# The packages and code included in this file are written exactly
# as they appear in AWS Lambda.
...
# Define one hour insert data function.
def insert_onehour_data(credentials, exchanges, period='3600'):
    ...
    Option to select a period ('60', '300', '3600'; set default='3600')
    """
    ...
    for exchange in exchanges:
        ...
        for trading_pair in exchanges[exchange]:
            ...
            try:
                ...
                # Define candles for data returned from the API and to only
                # include the actual candle.
                candles = response['result'][period]

                # Retrieve candles already in database. Collect roughly 24
                # hours worth of candles.
                cur.execute('''SELECT closing_time FROM {schema}.
                               {table_name} ORDER BY closing_time DESC LIMIT 30
                            '''.format(schema=schema, table_name=table_name))

                # Save those candles to results.
                results = cur.fetchall()

                # Select only the timestamps from the returned candles and
                # save to timestamps.
                timestamps = [result[0] for result in results]

                # Take most recent 24 hours worth of candles (24 candles) of
                # the candles returned by the API.
                # Ignore final candle because it is still open.
                completed_candles = candles[-25:-1]
...
def lambda_handler(event, context):
    period = '3600'
    insert_onehour_data(credentials, exchanges, period)
    return 'Success!'
