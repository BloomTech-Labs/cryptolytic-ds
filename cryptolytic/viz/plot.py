...
def candlestick(df):
    """Expects a dataframe with open, high, low, close data.
    Plots candlesticks using plotly."""
    if df.shape[0] > 10000:
...
def plot_all_candlesticks(df):
    """
    Will plot a candlestick chart for each api, exchange,
...
def plot_arbitration(df):
    ...
    ax2.plot(df.index, d.denoise(df.arb_signal, 20),
             color=color, label='Signal')
