...
# This lambda function is created for
# AWS Lambda and runs in a cloud9 environment
# It generates predictions with all trade
# recommender models from an S3 bucket
# and inserts them into the trp table
# under the prediction schema in the database.
...
coinbase_pro_table_list = [
    'coinbase_pro_' + pair for pair in coinbase_pro_pairs
    ]
...
def retrieve_data(cursor, table_name, schema):
    """ Retrieves data from a database where
    ...
    result = result.rename(
        columns={0: 'closing_time', 1: 'open', 2: 'high', 3: 'low', 4: 'close',
                 5: 'base_volume'})
...
def change_ohlcv_time(df, period):
    """ Changes the time period on cryptocurrency ohlcv data.
        Period is a string denoted by
        'time_in_minutesT'(ex: '1T', '5T', '60T')."""
...
def engineer_features(df, period):
    """ Takes in a dataframe of 1 hour cryptocurrency trading data
        and returns a new dataframe with selected period,
        new technical analysis features,
        selects features for modeling, and keeps the last row
        (which is the most current time).
        The function returns a dataframe of inputs for the model and
        the prediction time
    """
    ...
    # Add feature to indicate gaps in data
    # (could be caused by exchange being down)
    ...
    df = add_all_ta_features(
        df, "open", "high", "low", "close", "base_volume", fillna=True
        )
    ...
    # Replace remaining NaN values with the mean of each respective column and
    # reset index
    df = df.apply(lambda x: x.fillna(x.mean()), axis=0)

    # Create a feature for close price difference (current close price minus
    # previous close price)/(current close price)
    df['close_diff'] = (
    ...
    # feature selection
    feature_columns = ['open', 'high', 'low', 'close', 'base_volume',
                       'nan_ohlc', 'year', 'month', 'day', 'volume_adi',
                       ...
                       'volatility_dchi', 'volatility_dcli', 'trend_macd',
                       'trend_macd_signal', 'trend_macd_diff',
                       'trend_ema_fast', 'trend_ema_slow', 'trend_adx_pos',
                       'trend_adx_neg', 'trend_vortex_ind_pos',
                       'trend_vortex_ind_neg', 'trend_vortex_diff',
                       'trend_trix', 'trend_mass_index', 'trend_cci',
                       'trend_dpo', 'trend_kst', 'trend_kst_sig',
                       'trend_kst_diff', 'trend_ichimoku_a',
...
def prediction_to_database(table_names):
    """ This function connects to a database, retrieves data, does FE, loads
        ...
        into trp table under prediction schema in database.
        There is a known bug where duplicates are not caught and there are
        duplicates being inserted in the table - has to be fixed """
    ...
    for table_name in table_names:
        print('---------------STARTING {table_name}-----------------'.format(
            table_name=table_name))
        # define exchange and trading_pair
        if len(table_name.split('_')) == 4:
            exchange = table_name.split('_')[0] /
            + '_' + table_name.split('_')[1]
            trading_pair = table_name.split('_')[2] /
            + '_' + table_name.split('_')[3]
        else:
            exchange = table_name.split('_')[0]
            trading_pair = table_name.split('_')[1] + '_' /
            + table_name.split('_')[2]
        ...
        select_query = ("""SELECT p_time FROM prediction.trp
                          ...
                          ORDER BY p_time desc LIMIT 100;""".format(
        ...
        if str(prediction_time) not in timestamps:
            ...
            # This needs to be done because each model has a different
            # period for prediction
            # That was a change made after this function was written
            # Steps:
            # 1- database table has to be changed so that it
            #    has a period column
            ...
            result = [str(prediction_time), str(
                dt.datetime.utcnow()), exchange, trading_pair, pred]
            print(result)

            # insert into db
            insert_query = """INSERT INTO prediction.trp
                              (p_time, c_time, exchange,
                              trading_pair, prediction)
                              VALUES (%s, %s, %s, %s, %s)"""
